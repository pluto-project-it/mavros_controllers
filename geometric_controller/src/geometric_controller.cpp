...
void geometricCtrl::getTargetFromTrajectory2(const mav_msgs::EigenTrajectoryPoint &command_trajectory)
{
  targetPos_ = command_trajectory.position_W;
...
{
  //--- Inizio Test ---
  // trajectory_msgs::MultiDOFJointTrajectoryPoint pt = msg.points[msg.points.size() - 1];
  ...
  for (size_t i = 0; i < msg.points.size(); i++)
  {
    ...
    if (pt.time_from_start < curr_time_from_start)
    {
      //ROS_INFO_STREAM("getTargetFromTrajectory. Processing points n. " << i);
